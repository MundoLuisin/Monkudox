...
{
	return (2.0 * near * farPlane) / (farPlane + near - (depth * 2.0 - 1.0) * (farPlane - near));
}

float logisticDepth(float depth, float steepness = 0.1f, float offset = 75.0f)
{
...
void main()
{
	...
	vec3 fogColor = vec3(0.35f, 0.55f, 0.85f);
	vec4 light = direcLight() * 0.7 + pointLight() * 0.3; 
	FragColor = light * (1.0f - depth) + vec4(depth * fogColor, 1.0f);
}